...
export async function createPost(form:FormData){if(!await isAuthenticated())redirect("/blog/login");const title=text(form,"title"),excerpt=text(form,"excerpt"),content=sanitizePostHtml(text(form,"content"));const plainText=content.replace(/<[^>]*>/g,"").trim();if(title.length<4||excerpt.length<10||plainText.length<30)redirect("/blog/admin?error=validation");const post=await getBlogRepository().create({title,excerpt,content});revalidatePath("/blog");redirect(`/blog/${post.slug}`)}
