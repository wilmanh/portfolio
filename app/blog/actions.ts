...
export async function publishToLinkedIn(form: FormData) {
  if (!(await isAuthenticated())) redirect("/blog/login");
  const postId = Number(form.get("postId"));
  const post = Number.isInteger(postId) ? await getBlogRepository().findById(postId) : null;
  if (!post) redirect("/blog");
  const repository = getSocialRepository();
  const connection = await repository.getConnection("linkedin");
  if (!connection) redirect("/blog/admin/social/linkedin?error=configuration");
  ...
  try {
    ...
    publicationError = error instanceof Error ? error.message : "Error de publicación";
  }

  if (publicationError) {
    await repository.savePublication({ postId: post.id, platform: "linkedin", externalId: null, status: "failed", error: publicationError, publishedAt: null });
    redirect(`/blog/${post.slug}?linkedin=error`);
  }

  await repository.savePublication({ postId: post.id, platform: "linkedin", externalId, status: "published", error: null, publishedAt: new Date().toISOString() });
  revalidatePath(`/blog/${post.slug}`);
  redirect(`/blog/${post.slug}?linkedin=published`);
}
