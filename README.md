...
Para subir imágenes en Vercel, crea y vincula un Blob Store al proyecto. La aplicación acepta el token `BLOB_READ_WRITE_TOKEN` o la configuración automática de la integración (`BLOB_STORE_ID` junto con `VERCEL_OIDC_TOKEN`). Después de añadir o vincular el almacenamiento, redepliega el proyecto para que las variables estén disponibles. `BLOB_WEBHOOK_PUBLIC_KEY` queda disponible para verificar callbacks de subidas directas si se habilita ese flujo en el futuro. En desarrollo, si no existe ninguna de esas configuraciones, las imágenes se guardan en `public/uploads`.
